...
%instTbl = (
  # R-type instructions are associated with the FUNC field
  # I-type and J-type instructions are associated with the OPCODE field 
...
sub forminstr {
  ...
  $op = hex ($op);
  if ($itype =~ /^r/) { # R-type instructions
    if($itype eq "r3"){ # Exit handling
      $src1 = 0;
      $src2 = 0;
      $dst = 0;
    } else { # R-type with valid sources/destination
      $src1 = &getreg ($a[2]);
      if ($itype eq "r") {
        $src2 = &getreg ($a[3]);
      } else{
